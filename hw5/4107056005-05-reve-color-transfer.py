# USAGE
# python 4107056005-05-reve-color-transfer.py --result ult1.bmp --sideinfodeci sideinfodeci.txt
...
def color_transfer(result, file_name, clip=True, preserve_paper=True):
   """
   ...
   with open(file_name,'r') as f:
      txt = f.read().split()
   # print(txt)
   lMeanSrc  = float(txt[0])
   aMeanSrc  = float(txt[1])
   bMeanSrc  = float(txt[2])
   lStdSrc  = float(txt[3])
   aStdSrc  = float(txt[4])
   bStdSrc  = float(txt[5])
   lMeanTar  = float(txt[6])
   aMeanTar  = float(txt[7])
   bMeanTar  = float(txt[8])
   lStdTar  = float(txt[9])
   aStdTar  = float(txt[10])
   bStdTar  = float(txt[11])
...
ap.add_argument("-txt", "--sideinfodeci", 
   help = "Path to the sideinfodeci")
ap.add_argument("-r", "--result", 
...
file_name = args["sideinfodeci"]

# transfer the color distribution from the source image
# to the target image
transfer = color_transfer(result, file_name, clip=args["clip"], preserve_paper=args["preservePaper"])
cv2.imwrite("yrcsou1.bmp", transfer)
